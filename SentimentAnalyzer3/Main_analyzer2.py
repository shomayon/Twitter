...
def process_tweet(tweet, localtime):
    ...
    # get correct time
    tw_date = localtime
    twitdata = tweet._json
    if twitdata["place"] is not None:
        latit = twitdata["place"]["bounding_box"]["coordinates"][0][0][1]
        longit = twitdata["place"]["bounding_box"]["coordinates"][0][0][0]
        tw_date = getTime(twitdata,latit,longit)
...
def get_tweets(api, username, limit):
    ...
    for status in tqdm(tweepy.Cursor(api.user_timeline, screen_name=username,tweet_mode='extended').items(limit),
        ...
        if geoinfo is None:
            geolocator = Nominatim(user_agent="mental_health")
            location = geolocator.geocode(info["user"]["location"])
            localtime = getTime(info, location.latitude, location.longitude)
        else:
            localtime = None
        process_tweet(status, localtime)
