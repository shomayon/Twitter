...
def get_all_tweets(screen_name):
    ...
    if path.exists():
        ...
        with open(path, 'r') as f:
            for row in csv.DictReader(f):
                ...
                if since_id is None:
                    ...
                    break
        newtweets = []
        recent_tweets = api.user_timeline(screen_name=screen_name, count=200, since_id=since_id, tweet_mode='extended',
                                          lang='en')
        ...
        if len(recent_tweets) == 0:
            ...
            df = pd.DataFrame()
            final = pd.DataFrame()
            col_to_use = ['Label', 'Tweet_ID', 'Tweet', 'Screen_Name', 'Description', 'User_Location', 'Time',
                          'Geo_Enabled', 'Place', 'Lat', 'Long']
            ...
            newdata['Place'] = list(
                map(lambda tweet: tweet.place.full_name if tweet.place != None else None, newtweets))
            newdata['Lat'] = list(
                map(lambda tweet: tweet.coordinates["coordinates"][1] if tweet.coordinates != None else None,
                    newtweets))
            newdata['Long'] = list(
                map(lambda tweet: tweet.coordinates["coordinates"][0] if tweet.coordinates != None else None,
                    newtweets))
        ...
        data['Lat'] = list(
            map(lambda tweet: tweet.coordinates["coordinates"][1] if tweet.coordinates != None else None, alltweets))
        data['Long'] = list(
            map(lambda tweet: tweet.coordinates["coordinates"][0] if tweet.coordinates != None else None, alltweets))
